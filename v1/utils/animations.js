 ...
 * @param {number} duration - The duration of the animation in milliseconds. Default is 300ms.
...
export const fadeOutAndSlideUp = (fadeAnim, slideAnim, duration = 300) => {
  return Animated.parallel([
    Animated.timing(fadeAnim, {
      toValue: 0,
      duration,
      useNativeDriver: true, // Ensure opacity is supported by native driver
    }),
    Animated.timing(slideAnim, {
      toValue: -50,
      duration,
      useNativeDriver: true, // Ensure translateY is supported by native driver
    }),
  ]);
};
